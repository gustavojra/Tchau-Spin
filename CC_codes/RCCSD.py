...
def update_energy(T1, T2, fock_OV, Voovv):
    ...
    CC_energy += 2.0*np.einsum('kldc, lkcd -> ', T2, Voovv, optimize = 'optimal')
    CC_energy -= np.einsum('lkcd, klcd -> ', T2, Voovv, optimize = 'optimal')
    CC_energy += 2.0*np.einsum('lc, kd, lkcd -> ', T1, T1, Voovv, optimize = 'optimal')
    CC_energy -= np.einsum('lc, kd, klcd -> ', T1, T1, Voovv, optimize = 'optimal')
    return CC_energy
...
def update_amp(T1, T2, f, V, d, D, info):
    ...
    newT1 += 1.5*np.einsum('ikcd, kadc -> ia', T2, Vovvv, optimize = 'optimal')
    newT1 += -2.0*np.einsum('kc, la, lkic -> ia', T1, T1, Vooov, optimize = 'optimal')
    newT1 += 0.5*np.einsum('kicd, kacd -> ia', T2, Vovvv, optimize = 'optimal')
    newT1 += 2.0*np.einsum('kc, id, kacd -> ia', T1, T1, Vovvv, optimize = 'optimal')
    newT1 += -2.0*np.einsum('lkac, lkic -> ia', T2, Vooov, optimize = 'optimal')
    newT1 += -0.5*np.einsum('ikcd, kacd -> ia', T2, Vovvv, optimize = 'optimal')
    newT1 += -0.5*np.einsum('kicd, kadc -> ia', T2, Vovvv, optimize = 'optimal')
    ...
    newT1 += -1.5*np.einsum('la, ikcd, lkcd -> ia', T1, T2, Voovv, optimize = 'optimal')
    ...
    newT1 += -0.5*np.einsum('la, kicd, klcd -> ia', T1, T2, Voovv, optimize = 'optimal')
    newT1 += np.einsum('kc, id, la, lkcd -> ia', T1, T1, T1, Voovv, optimize = 'optimal')
    newT1 += 0.5*np.einsum('la, kicd, lkcd -> ia', T1, T2, Voovv, optimize = 'optimal')
    newT1 += 0.5*np.einsum('la, ikcd, klcd -> ia', T1, T2, Voovv, optimize = 'optimal')
    ...
    newT2 -= np.einsum('ka, ijkb -> ijab', T1, Vooov, optimize = 'optimal')
    newT2 += np.einsum('jc, icab -> ijab', T1, Vovvv, optimize = 'optimal')
    newT2 -= np.einsum('kc, ka, ijcb -> ijab', fock_OV, T1, T2, optimize = 'optimal')
    newT2 -= np.einsum('kc, kb, ijac -> ijab', fock_OV, T1, T2, optimize = 'optimal')
    newT2 -= np.einsum('kc, jc, ikab -> ijab', fock_OV, T1, T2, optimize = 'optimal')
    newT2 += np.einsum('ic, jcba -> ijab', T1, Vovvv, optimize = 'optimal')
    newT2 -= np.einsum('kb, jika -> ijab', T1, Vooov, optimize = 'optimal')
    newT2 -= np.einsum('kc, ic, kjab -> ijab', fock_OV, T1, T2, optimize = 'optimal')
    ...
    newT2 -= np.einsum('jc, kb, kica -> ijab', T1, T1, Voovv, optimize = 'optimal')
    newT2 -= np.einsum('kiac, kjcb -> ijab', T2, Voovv, optimize = 'optimal')
    newT2 += np.einsum('lkab, ijlk -> ijab', T2, Voooo, optimize = 'optimal')
    newT2 -= np.einsum('kjcb, icka -> ijab', T2, Vovov, optimize = 'optimal')
    newT2 -= np.einsum('jc, ka, ickb -> ijab', T1, T1, Vovov, optimize = 'optimal')
    newT2 -= np.einsum('ic, ka, kjcb -> ijab', T1, T1, Voovv, optimize = 'optimal')
    newT2 -= np.einsum('ic, kb, jcka -> ijab', T1, T1, Vovov, optimize = 'optimal')
    ...
    newT2 += 2.0*np.einsum('kc, ijdb, kacd -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 -= np.einsum('ic, kjdb, kacd -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 -= np.einsum('ka, ijdc, kbdc -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 -= np.einsum('ic, kjbd, kadc -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 += np.einsum('jc, ka, lb, klic -> ijab', T1, T1, T1, Vooov, optimize = 'optimal')
    newT2 += np.einsum('lb, ikac, kljc -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 -= np.einsum('ka, ljcb, klic -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 -= np.einsum('jc, kiad, kbdc -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 -= np.einsum('kc, ijdb, kadc -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 += np.einsum('ic, ka, lb, lkjc -> ijab', T1, T1, T1, Vooov, optimize = 'optimal')
    newT2 += -2.0*np.einsum('kc, ilab, lkjc -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 -= np.einsum('jc, ikdb, kacd -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 += np.einsum('ic, lkab, kljc -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 -= np.einsum('jc, ikad, kbcd -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 += np.einsum('kc, ilab, kljc -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 += np.einsum('lb, kiac, lkjc -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 -= np.einsum('ka, jlbc, klic -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 += np.einsum('ka, ilcb, lkjc -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 += np.einsum('lb, kjac, klic -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 -= np.einsum('ic, jd, kb, kadc -> ijab', T1, T1, T1, Vovvv, optimize = 'optimal')
    newT2 -= np.einsum('kb, ijcd, kadc -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 += np.einsum('ic, jkbd, kadc -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 += -2.0*np.einsum('lb, ikac, lkjc -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 += 2.0*np.einsum('jc, ikad, kbdc -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 += np.einsum('ka, ljcb, lkic -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 += -2.0*np.einsum('kc, ljab, lkic -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 += 2.0*np.einsum('kc, ijad, kbcd -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 += np.einsum('ic, kjdb, kadc -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 -= np.einsum('ic, jd, ka, kbcd -> ijab', T1, T1, T1, Vovvv, optimize = 'optimal')
    newT2 += np.einsum('ka, ljbc, klic -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 -= np.einsum('ic, kjad, kbcd -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 -= np.einsum('kc, ijad, kbdc -> ijab', T1, T2, Vovvv, optimize = 'optimal')
    newT2 += np.einsum('jc, klab, klic -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 += np.einsum('kc, ljab, klic -> ijab', T1, T2, Vooov, optimize = 'optimal')
    newT2 += -2.0*np.einsum('jc, lb, ikad, lkcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 -= np.einsum('ijac, lkbd, lkcd -> ijab', T2, T2, Voovv, optimize = 'optimal')
    newT2 += np.einsum('kc, jd, ilab, lkcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 += -2.0*np.einsum('ikac, ljbd, klcd -> ijab', T2, T2, Voovv, optimize = 'optimal')
    newT2 += np.einsum('jc, lb, kiad, lkcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 += -2.0*np.einsum('kc, la, ijdb, klcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 -= np.einsum('ic, ka, ljdb, klcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    ...
    newT2 += np.einsum('ic, ka, ljbd, klcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 += np.einsum('jc, ka, ildb, lkcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 -= np.einsum('ijac, kldb, lkcd -> ijab', T2, T2, Voovv, optimize = 'optimal')
    ...
    newT2 += np.einsum('kc, lb, ijad, lkcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    ...
    newT2 += np.einsum('kc, id, ljab, lkcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 += -2.0*np.einsum('kc, jd, ilab, klcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 += np.einsum('kiac, ljbd, klcd -> ijab', T2, T2, Voovv, optimize = 'optimal')
    newT2 += -1.5*np.einsum('ikdc, ljab, klcd -> ijab', T2, T2, Voovv, optimize = 'optimal')
    newT2 += np.einsum('ic, lb, kjad, klcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 -= np.einsum('ikac, ljdb, lkcd -> ijab', T2, T2, Voovv, optimize = 'optimal')
    newT2 += -2.0*np.einsum('kc, id, ljab, klcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 += 0.5*np.einsum('kicd, ljab, lkcd -> ijab', T2, T2, Voovv, optimize = 'optimal')
    newT2 += np.einsum('ic, jd, klab, klcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 += np.einsum('ka, lb, ijcd, klcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 += -0.5*np.einsum('jkcd, ilab, lkcd -> ijab', T2, T2, Voovv, optimize = 'optimal')
    newT2 += np.einsum('jc, lb, ikad, klcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 += -2.0*np.einsum('kc, lb, ijad, klcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    ...
    newT2 -= np.einsum('ic, ka, jlbd, klcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 += np.einsum('kc, la, ijdb, lkcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
    newT2 += np.einsum('ijac, klbd, lkcd -> ijab', T2, T2, Voovv, optimize = 'optimal')
    newT2 += np.einsum('ic, ka, ljdb, lkcd -> ijab', T1, T1, T2, Voovv, optimize = 'optimal')
